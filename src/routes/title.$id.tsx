        ...
        {streamLoading ? (
          ...
          <div className="flex aspect-video w-full flex-col items-center justify-center gap-3 bg-gray-950 text-white">
            ...
            <button
              ...
              className="rounded-full bg-primary px-4 py-2 text-sm font-bold"
            >
              Dismiss
            </button>
          </div>
            ...
            <div className="relative z-10 flex flex-col items-center gap-4 text-center px-6">
              <button
                onClick={() =>
                  ...
                  handlePlay(seasonQ.data.episodes[0].episode_number, seasonQ.data.episodes[0].name)
                }
              ...
              <p className="text-[11px] text-white/40">
                {user ? "Native HLS · Ad-free · Custom controls" : "Sign in to watch"}
              </p>
              {!user && (
                <button
                  onClick={() => setShowAuth(true)}
                  className="rounded-full border border-white/20 bg-white/10 px-5 py-2 text-sm font-bold text-white backdrop-blur hover:bg-primary transition"
                >
                  Sign In
                </button>
              )}
        ...
        {playingEp && !streamLoading && (
          <div className="flex items-center justify-between border-t border-white/5 bg-gray-950 px-4 py-2">
            <div className="flex items-center gap-2">
              ...
              {activeStream && (
                <span
                  className={`rounded-full px-2 py-0.5 text-[9px] font-black ${activeStream.sources.length > 0 ? "bg-green-500/20 text-green-400" : "bg-orange-500/20 text-orange-400"}`}
                >
            ...
            <Link
              to="/"
              className="flex items-center gap-1 text-[10px] font-bold text-white/30 hover:text-primary transition"
            >
          ...
          <div className="flex gap-4">
            {data.poster_path && (
              <img
                ...
                alt={title}
                className="h-24 w-16 shrink-0 rounded-2xl object-cover shadow-lg"
              />
            )}
            <div className="min-w-0 flex-1">
              <h1 className="text-xl font-black leading-tight text-gray-900">{title}</h1>
              {data.tagline && (
                <p className="mt-0.5 line-clamp-1 text-xs italic text-gray-400">"{data.tagline}"</p>
              )}
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                {data.genres?.slice(0, 3).map((g: any) => (
                  <span
                    key={g.id}
                    className="rounded-full bg-gray-100 px-2.5 py-0.5 text-[10px] font-semibold text-gray-500"
                  >
                    {g.name}
                  </span>
                ))}
          ...
          <section className="border-b">
            ...
            <div className="space-y-2 px-4 pb-4">
              ...
              {seasonQ.data?.episodes.map((ep) => (
                <div
                  key={ep.id}
                  onClick={() => handlePlay(ep.episode_number, ep.name)}
            ...
            <div className="flex gap-4 overflow-x-auto scrollbar-hide pb-1">
              {cast.map((c: any) => (
                <div key={c.id} className="flex w-16 shrink-0 flex-col items-center text-center">
                  ...
                  <p className="mt-1 line-clamp-2 text-[10px] font-semibold leading-tight text-gray-700">
                    {c.name}
                  </p>
                </div>
              ))}
