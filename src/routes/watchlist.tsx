...
function WatchlistPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-8">
      ...
      <p className="mt-2 text-sm text-muted-foreground">{list.length} title{list.length === 1 ? "" : "s"} saved</p>

      {list.length === 0 ? (
        <div className="mt-16 rounded-2xl border bg-card p-12 text-center">
          <p className="text-lg font-semibold">No saves yet</p>
          <p className="mt-1 text-sm text-muted-foreground">Tap the bookmark on any title to add it here.</p>
          ...
          {list.map((t) => (
            <div key={t.id} className="group relative">
              <Link to="/title/$id" params={{ id: String(t.id) }} className="block">
                <div className="relative overflow-hidden rounded-xl bg-muted shadow-card transition group-hover:-translate-y-1 group-hover:shadow-lift" style={{ aspectRatio: "2/3" }}>
                  {t.poster_path && <img src={img(t.poster_path)} alt={t.name} className="h-full w-full object-cover" loading="lazy" />}
