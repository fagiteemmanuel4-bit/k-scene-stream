...
import {
  LogOut, Settings, Bookmark, Clock, Download, Globe,
  Wifi, LogIn, Trash2, Play, Star, Flame, Trophy, Zap
} from "lucide-react";
...
const COUNTRIES = ["Nigeria", "South Korea", "United States", "United Kingdom", "Canada", "Ghana", "Kenya", "South Africa", "India", "Australia", "Brazil", "Japan", "Philippines"];
const QUALITIES = [
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-6">
            ...
            <div className="mt-4 flex items-center gap-3 rounded-2xl bg-gradient-to-r from-primary to-orange-400 p-4 text-white">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs opacity-80">
                  {streakCount >= 30 ? "Legendary 🏆" : streakCount >= 14 ? "Dedicated fan 🔥" : streakCount >= 7 ? "K-drama addict 💜" : "Keep it going!"}
                </p>
              </div>
          ...
          {TABS.map(t => (
            <button
              ...
              className={`flex flex-1 flex-col items-center gap-0.5 rounded-xl py-2 text-[10px] font-bold transition ${
                activeTab === t.key ? "bg-primary/10 text-primary" : "text-gray-400 hover:text-gray-700"
              }`}
            >
              <t.icon className="h-4 w-4" />
              {t.label}
              {t.count !== undefined && t.count > 0 && (
                <span className={`rounded-full px-1.5 py-0.5 text-[9px] font-black ${activeTab === t.key ? "bg-primary text-white" : "bg-gray-100 text-gray-500"}`}>
                  {t.count}
                </span>
              )}
            </button>
      ...
      <div className="mx-auto max-w-xl px-4 pt-5">
        ...
        {activeTab === "watchlist" && (
          watchlist.length === 0 ? (
            <EmptyState emoji="🔖" title="Nothing saved yet" sub="Tap + on any drama to save it here." />
          ) : (
            <div className="grid grid-cols-3 gap-3">
              {watchlist.map(item => (
                <Link key={item.id} to="/title/$id" params={{ id: String(item.id) }} className="group relative overflow-hidden rounded-xl bg-gray-100">
                  <div style={{ aspectRatio: "2/3" }}>
                    {item.poster_path ? (
                      ...
                      <div className="grid h-full w-full place-items-center text-2xl bg-gray-200">🎬</div>
                    )}
                  </div>
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-2">
                    <p className="line-clamp-2 text-[10px] font-bold text-white leading-tight">{item.name}</p>
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {item.season && item.episode && (
                        <p className="text-xs text-gray-500">S{item.season} · E{item.episode}</p>
                      )}
                      <p className="text-[10px] text-gray-400">{new Date(item.watchedAt).toLocaleDateString()}</p>
                    </div>
        ...
        {activeTab === "downloads" && (
          downloads.length === 0 ? (
            <EmptyState emoji="⬇️" title="No downloads" sub="Use the Download button inside the player." />
          ) : (
            <div className="space-y-3">
              {downloads.map((item, i) => (
                <div key={i} className="flex items-center gap-3 rounded-2xl border border-gray-100 bg-white p-3 shadow-sm">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-bold text-gray-900">{item.name}</p>
                    {item.season && item.episode && (
                      <p className="text-xs text-gray-500">S{item.season} · E{item.episode} · {item.quality}</p>
                    )}
                    <p className="text-[10px] text-gray-400">{new Date(item.downloadedAt).toLocaleDateString()}</p>
                  </div>
        ...
        {activeTab === "settings" && (
          <div className="space-y-4">
            <SettingsCard title="Country / Region" icon={<Globe className="h-4 w-4 text-primary" />}>
              <select
            ...
            <SettingsCard title="Streaming Quality" icon={<Wifi className="h-4 w-4 text-primary" />}>
              {QUALITIES.map(q => (
                <label key={q.value} className="flex cursor-pointer items-center gap-3 py-1">
                  <div className={`h-4 w-4 rounded-full border-2 flex items-center justify-center ${settings.streamingQuality === q.value ? "border-primary" : "border-gray-300"}`}>
