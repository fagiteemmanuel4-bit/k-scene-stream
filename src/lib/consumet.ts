...
async function fromAutoembed(tmdbId: number, s: number, e: number): Promise<StreamResult | null> {
  ...
  const sources: StreamSource[] = data.sources
    ...
    .map((x: any) => ({
      url: x.file,
      quality: x.label || "HD",
      isM3U8: isM3U8(x.file),
      label: x.label || "AutoEmbed",
    }));
...
async function fromSmashy(tmdbId: number, s: number, e: number): Promise<StreamResult | null> {
  ...
  return {
    sources: [{ url: src, quality: "HD", isM3U8: isM3U8(src), label: "Smashy HD" }],
    subtitles: [],
    fallbackEmbeds: [],
  };
}
...
export async function getMovieStream(tmdbId: number): Promise<StreamResult> {
  const results = await Promise.allSettled([
    tryFetch(
      `https://player.autoembed.cc/api/source/movie/${tmdbId}`,
      "https://player.autoembed.cc/",
    ).then(
      (d) =>
        d?.sources
          ?.filter((s: any) => isDirect(s.file))
          .map((s: any) => ({
            url: s.file,
            quality: s.label || "HD",
            isM3U8: isM3U8(s.file),
            label: s.label || "AutoEmbed",
          })) as StreamSource[] | undefined,
    ),
    tryFetch(`https://embed.su/api/source/movie/${tmdbId}`, "https://embed.su/").then(
      (d) =>
        d?.sources
          ?.filter((s: any) => isDirect(s.file))
          .map((s: any) => ({
