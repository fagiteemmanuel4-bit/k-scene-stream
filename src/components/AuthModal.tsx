...
export function AuthModal({ onClose, defaultTab = "login" }: Props) {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    const { error } =
      tab === "login" ? await signIn(email, password) : await signUp(email, password, name);
    setLoading(false);
    if (error) {
      setError(error);
      return;
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative w-full max-w-sm rounded-t-3xl sm:rounded-3xl border bg-white p-6 shadow-hero sm:p-8">
        <button
          onClick={onClose}
          className="absolute right-4 top-4 grid h-8 w-8 place-items-center rounded-full bg-gray-100 text-gray-500 hover:bg-gray-200 transition"
        >
          <X className="h-4 w-4" />
        </button>
        ...
        <div className="mt-4 space-y-3">
          ...
          {error && (
            <p className="rounded-xl bg-red-50 px-4 py-2 text-xs font-medium text-red-600">
              {error}
            </p>
          )}
